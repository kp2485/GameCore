...
@Suite struct AbilityStatusTargetingTests {
    ...
    @Test func applyStatusStacksOrRefreshes() {
        ...
        let stagger = Ability<Character>(name: "Stagger", costMP: 0, targeting: .singleEnemy) {
    ...
    @Test func stackingStatusIncrementsStacks() {
        ...
        var rt = CombatRuntime<Character>(tick: 0, encounter: enc, currentIndex: 0, side: .allies, rngForInitiative: SeededPRNG(seed: 1))
        var rng: any Randomizer = SeededPRNG(seed: 9)

        let stacking = Status(.regen, duration: 3, stacks: 1, rule: .stack(3))
        let ability = Ability<Character>(name: "Regen", costMP: 0, targeting: .singleAlly) {
    ...
    @Test func statusTicksAndExpires() {
        let hero = char("Hero", [.spd: 5])
        let gob  = char("Gob",  [.spd: 4])
        let enc = Encounter(allies: [Combatant(base: hero, hp: 30)], foes: [Combatant(base: gob, hp: 18)])
        var rt = CombatRuntime<Character>(tick: 0, encounter: enc, currentIndex: 0, side: .allies, rngForInitiative: SeededPRNG(seed: 1))

        // apply a 1-turn status
        rt.apply(status: Status(.staggered, duration: 1), to: hero)
        let ev = rt.tickStatuses()
        #expect(ev.contains { $0.kind == .statusExpired })
        #expect(rt.status(of: .staggered, on: hero) == nil)
    }

    @Test func targetingResolvesCorrectSets() {
        let a = char("A", [.spd: 5])
        let b = char("B", [.spd: 4])
        let c = char("C", [.spd: 6])

        let enc = Encounter(allies: [Combatant(base: a, hp: 10)], foes: [Combatant(base: b, hp: 10), Combatant(base: c, hp: 0)]) // c is dead
        let rt = CombatRuntime<Character>(tick: 0, encounter: enc, currentIndex: 0, side: .allies, rngForInitiative: SeededPRNG(seed: 1))
        ...
        #expect(resolver.resolve(mode: .singleEnemy, state: rt, rng: &rng).map(\.name) == ["B"])
        #expect(resolver.resolve(mode: .allEnemies, state: rt, rng: &rng).map(\.name) == ["B"]) // C excluded (dead)
        #expect(resolver.resolve(mode: .singleAlly, state: rt, rng: &rng).map(\.name) == ["A"])
        #expect(resolver.resolve(mode: .allAllies, state: rt, rng: &rng).map(\.name) == ["A"])
    }

    @Test func castAbilityRunsEffects() {
        let hero = char("Mage", [.int: 8, .spd: 6])
        let gob  = char("Gob",  [.vit: 2, .spd: 4])

        let enc = Encounter(allies: [Combatant(base: hero, hp: 20)], foes: [Combatant(base: gob, hp: 12)])
        var rt = CombatRuntime<Character>(tick: 0, encounter: enc, currentIndex: 0, side: .allies, rngForInitiative: SeededPRNG(seed: 1))
        var rng: any Randomizer = SeededPRNG(seed: 10)

        let firebolt = Ability<Character>(name: "Firebolt", costMP: 3, targeting: .singleEnemy) {
        ...
        #expect(after <= before)
        #expect(ev.contains { $0.kind == .statusApplied })
    }
}
